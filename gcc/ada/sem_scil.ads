...
package Sem_SCIL is
   ...
   --  SCIL (SofCheck Intermediate Language) is produced by the CodePeer back
   --  end (aka gnat2scil). For some constructs (tagged type declarations,
   --  dispatching calls, classwide membership tests), the CodePeer back end
   --  needs to locate certain nodes in the tree. To allow CodePeer to do this
   --  without introducing unwanted dependencies on the details of the FE's
   --  expansion strategies, SCIL_Nodes are generated.

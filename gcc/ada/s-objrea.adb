...
package body System.Object_Reader is
   ...
   procedure Read (F : ICS.FILEs; Addr : Address; Size : uint32);
   ...
   package body PECOFF_Ops is
      ...
      begin
         ...
         Res.Symtab_Last  := Res.Symtab +
                               Offset (Hdr.NumberOfSymbols) *
                                 (Symtab_Entry'Size / SSU);

         --  Save some offsets

         Seek (Res, Signature_Loc_Offset);
         Hdr_Offset := Offset (uint32'(Read (Res)));
         Res.Sectab := Hdr_Offset +
                         Size_Of_Standard_Header_Fields +
                         Offset (Hdr.SizeOfOptionalHeader);

         --  Read optional header and extract image base

         Seek (Res, Hdr_Offset + Size_Of_Standard_Header_Fields);

         if Res.Format = PECOFF then
            declare
               Opt_32 : Optional_Header_PE32;
            begin
               Read (F, Opt_32'Address, uint32 (Opt_32'Size / SSU));
            ...
            begin
               Read (F, Opt_64'Address, uint32 (Opt_64'Size / SSU));
   ...
   is
      ...
      Raw_Len : constant size_t := size_t (CRTL.strlen (Raw'Address));

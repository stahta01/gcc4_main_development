      ...
      Int gigi_operating_mode)
{
  ...
  flag_debug_instances = (Generate_SCO_Instance_Table != 0);
  ...
  /* If the frontend built the SCO table, use it to identify multi-statement
     lines.  */
  if (SCO_Unit_Table_Last_Val >= 1)
    {
      const unsigned count = LINEMAPS_USED (line_table, false);
      /* Set a bit for all lines that contain at least one statement.  */
      bitmap stmt_lines;

      unsigned i;
      SCO_Unit_Index unit_no;
      int sco_no;

      bitmap_obstack_initialize (NULL);
      multi_stmt_lines = (bitmap *) xmalloc (count * sizeof (bitmap));
      for (i = 0; i < count; ++i)
	multi_stmt_lines[i] = BITMAP_ALLOC (NULL);
      stmt_lines = BITMAP_ALLOC (NULL);

      for (unit_no = 1;
	   unit_no <= SCO_Unit_Table_Last_Val;
	   ++unit_no)
	{
	  const unsigned unit_offset = unit_no - SCO_Unit_Table_Min;
	  const struct SCO_Unit_Table_Entry *unit
	    = SCO_Unit_Table + unit_offset;
	  const unsigned map_no = unit->File_Index - 1;

	  for (sco_no = unit->From; sco_no <= unit->To; ++sco_no)
	    {
	      const unsigned sco_offset = sco_no - SCO_Table_Min;
	      const struct SCO_Table_Entry *sco = SCO_Table + sco_offset;
	      Line_Number_Type line;

	      if (sco->C1 != 'S')
		continue;

	      for (line = sco->From.Line; line <= sco->To.Line; ++line)
		{
		  if (bitmap_bit_p (stmt_lines, line))
		    bitmap_set_bit (multi_stmt_lines[map_no], line);
		  else
		    bitmap_set_bit (stmt_lines, line);
		}
	    }
	}
      BITMAP_FREE (stmt_lines);
    }
  ...
  switch (pragma_id)
    ...
    case Pragma_Loop_Optimize:
      for (gnat_temp = First (Pragma_Argument_Associations (gnat_node));
	   ...
	   gnat_temp = Next (gnat_temp))
	{
	  tree gnu_loop_stmt = VEC_last (loop_info, gnu_loop_stack)->stmt;

	  switch (Chars (Expression (gnat_temp)))
	    {
	    case Name_Ivdep:
	      /* Hint not supported with the GCC 4.7 back-end.  */
	      break;
	  ...
	  if (TREE_CODE (gnu_expr) == ADDR_EXPR
		...
		FUNC_ADDR_BY_DESCRIPTOR (gnu_expr) = 1;
		 ...
		 No_Implicit_Dynamic_Code restriction.  */
	      else if (USE_RUNTIME_DESCRIPTORS != 0)
	...
	*gnu_else_ptr = build_stmt_group (Else_Statements (gnat_node), false);
	...
	= build2 (EXIT_STMT, void_type_node,
		  ...
		  (Present (Name (gnat_node))
		   ...
		   : LOOP_STMT_LABEL
		     (VEC_last (loop_info, gnu_loop_stack)->stmt)));
      break;
	...
	if (Present (Expression (gnat_node)))
	  {
		...
		if (!TYPE_CI_CO_LIST (gnu_subprog_type) && optimize_cfg)
		  {
...
gnat_gimplify_stmt (tree *stmt_p)
{
  ...
  switch (TREE_CODE (stmt))
	...
	*stmt_p = NULL_TREE;
	...
	GOTO_LOOP_NO_UNROLL (t) = LOOP_STMT_NO_UNROLL (stmt);
 	GOTO_LOOP_UNROLL (t) = LOOP_STMT_UNROLL (stmt);
	GOTO_LOOP_NO_VECTOR (t) = LOOP_STMT_NO_VECTOR (stmt);
	GOTO_LOOP_VECTOR (t) = LOOP_STMT_VECTOR (stmt);

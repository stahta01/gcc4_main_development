...
gnat_post_options (const char **pfilename ATTRIBUTE_UNUSED)
{
  ...
  /* SCOs are generated by the Ada front-end if requested, so disable
     the gimple-based SCO generator.  */
  flag_dump_scos = 0;
...
{
  tree gnu_index_type, ada_index_type;
  ...
  info->ndimensions = 1;
  info->element_type = TREE_TYPE (type);
  info->base_decl = NULL_TREE;
  info->data_location = NULL_TREE;
  info->allocated = NULL_TREE;
  info->associated = NULL_TREE;

  gnu_index_type = TYPE_DOMAIN (type);
  ada_index_type = TYPE_INDEX_TYPE (gnu_index_type);

  info->dimen[0].bounds_type = ada_index_type;
  info->dimen[0].lower_bound = TYPE_MIN_VALUE (gnu_index_type);
  info->dimen[0].upper_bound = TYPE_MAX_VALUE (gnu_index_type);

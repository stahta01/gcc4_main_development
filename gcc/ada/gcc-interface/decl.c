	  ...
	  if (IN (Ekind (gnat_base_type), Record_Kind)
		  ...
		  && all_constant_pos)
		{
		 ...
		 to create the nest of variant parts from the old nest.  */
	      else if (gnu_variant_list && !selected_variant)
		{
...
DEF_VEC_O(vinfo_t);
DEF_VEC_ALLOC_O_STACK(vinfo_t);
#define VEC_vinfo_t_stack_alloc(alloc) VEC_stack_alloc(vinfo_t, alloc)
		      ...
		      tree first_free_pos, tree *p_gnu_rep_list)
{
  ...
  if (Present (variant_part))
	...
	= concat_name (get_identifier (Get_Name_String (Chars (gnat_discr))),
      ...
      VEC(vinfo_t, stack) *variant_types = VEC_alloc (vinfo_t, stack, 16);
      vinfo_t *gnu_variant;
	   ...
	   variant = Next_Non_Pragma (variant))
	{
	  ...
	  VEC_safe_push (vinfo_t, stack, variant_types, &vinfo);
	...
	this_first_free_pos = round_up (this_first_free_pos, variants_align);
	 ...
	 built with their qualifier, unless this is an unchecked union.  */
      FOR_EACH_VEC_ELT (vinfo_t, variant_types, i, gnu_variant)
	{
      ...
      VEC_free (vinfo_t, stack, variant_types);

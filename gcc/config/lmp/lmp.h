/* Definitions of target machine for LMP.
...
#define TARGET_CPU_CPP_BUILTINS()		\
  do						\
    {						\
      builtin_define ("__LMP__");		\
      if (TARGET_MCM)				\
	builtin_define ("__LMP_ARCH_MCM__");	\
      if (TARGET_BMI)				\
	builtin_define ("__LMP_ARCH_BMI__");	\
    }						\
  while (0)

/* Recast the cpu class to be the cpu attribute.
   Every file includes us, but not every file includes insn-attr.h.  */
#define lmp_cpu_attr ((enum attr_cpu) lmp_cpu)
...
#define INIT_EXPANDERS lmp_init_expanders ()
...
#define LMP_STACK_ALIGN(LOC) (((LOC) + 3) & ~3)
...
#define DATA_ALIGNMENT(TYPE,ALIGN) lmp_data_alignment (TYPE, ALIGN)
...
#define CONSTANT_ALIGNMENT(EXP,ALIGN) \
  lmp_data_alignment (TREE_TYPE (EXP), ALIGN)
...
#define LOCAL_ALIGNMENT(TYPE,ALIGN) lmp_data_alignment (TYPE, ALIGN)
...
#define GP_REGISTER_P(REG) ((REG) <= 31)

#define MDB_REGNUM 32
#define MDC_REGNUM 33

#define FP_REG_FIRST 34
#define FP_REG_LAST  49
#define FP_RETURN (FP_REG_FIRST + 1)
#define FP_REGISTER_P(REG) (FP_REG_FIRST <= (REG) && (REG) <= FP_REG_LAST)
...
#define HARD_REGNO_RENAME_OK(OLD_REG, NEW_REG) \
  lmp_hard_regno_rename_ok (OLD_REG, NEW_REG)
...
#define REG_CLASS_NAMES \
 {"NO_REGS", "MDB", "MDC", "FP_REGS", "R1", "R2", "R3", "SIBCALL_REGS", \
...
#define REG_CLASS_CONTENTS {                     \
    ...
    {0xffffffff, 0x000c0000}, /* GENERAL_REGS */ \
...
#define DYNAMIC_CHAIN_ADDRESS(FRAMEADDR) lmp_dynamic_chain_address (FRAMEADDR)
...
#define RETURN_ADDR_RTX(COUNT,FRAMEADDR) lmp_return_addr_rtx (COUNT, FRAMEADDR)
...
#define EH_RETURN_HANDLER_RTX lmp_eh_return_handler_rtx ()
...
#define FRAME_POINTER_REGNUM 51
...
#define INITIAL_ELIMINATION_OFFSET(FROM, TO, OFFSET) \
  (OFFSET = lmp_initial_elimination_offset (FROM, TO))
...
#define FP_ARG_FIRST (FP_REG_FIRST + 1)
...
#define CUMULATIVE_ARGS struct lmp_args
...
#define FUNCTION_VALUE_REGNO_P(N) \
  ((N) == GP_RETURN || (TARGET_FPU && (N) == FP_RETURN))
...
/* `EPILOGUE_USES (REGNO)'
   ...
   needed. */
#define EPILOGUE_USES(REGNO) lmp_epilogue_uses (REGNO)

/* Generating Code for Profiling

   These macros will help you generate code for profiling. */

#define PROFILE_HOOK(LABEL) lmp_profile_hook ()
...
do									\
{									\
  rtx new_x = lmp_legitimize_reload_address ((AD), (MODE), (OPNUM),	\
					(int) (TYPE), (IND));		\
  if (new_x)								\
    {									\
      (AD) = new_x;							\
...
#define MOVE_RATIO(speed) ((speed) ? 7 : 3)
...
/* `CASE_DROPS_THROUGH'
   ...
   branches.  The default is four for machines with a `casesi'
...
extern int lmp_indent_opcode;

#define ASM_OUTPUT_OPCODE(FILE, PTR)	\
  do {					\
    if (lmp_indent_opcode)		\
      {					\
	putc (' ', FILE);		\
	lmp_indent_opcode = 0;		\
      }					\

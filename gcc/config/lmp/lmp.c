...
struct GTY (()) machine_function
{
  ...
  /* True if we created an rtx which relies on the frame pointer.  */
  bool frame_needed;
};

#define lmp_frame_size			cfun->machine->frame_size
#define lmp_reg_parm_save_area_size 	cfun->machine->reg_parm_save_area_size
#define lmp_frame_needed		cfun->machine->frame_needed

/* 1 if the next opcode is to be specially indented.  */
int lmp_indent_opcode = 0;
...
static void lmp_output_address (FILE *, enum machine_mode, rtx);
static tree lmp_handle_interrupt_attr (tree *, tree, tree, int, bool *);
...
static const struct attribute_spec lmp_attribute_table[] =
{
  /* { name, min_len, max_len, decl_req, type_req, fn_type_req, handler,
       affects_type_identity } */
  {"interrupt", 0, 0, true, false, false, lmp_handle_interrupt_attr, false},
  {NULL, 0, 0, false, false, false, NULL, false}
};

static struct machine_function *lmp_init_machine_status (void);

/* Target hooks and TARGET_INITIALIZER  */

static bool lmp_pass_by_reference (cumulative_args_t, enum machine_mode,
				   const_tree, bool);

static rtx lmp_function_arg (cumulative_args_t, enum machine_mode, const_tree,
			     bool);

static void lmp_function_arg_advance (cumulative_args_t, enum machine_mode,
				      const_tree, bool);

static bool lmp_return_in_memory (const_tree, const_tree fntype);

static rtx lmp_function_value (const_tree, const_tree fn_decl_or_type, bool);

static rtx lmp_libcall_value (enum machine_mode, const_rtx);

static void lmp_setup_incoming_varargs (cumulative_args_t, enum machine_mode,
				        tree, int *, int);

static void lmp_va_start (tree valist, rtx nextarg);

static tree lmp_gimplify_va_arg (tree valist, tree type, gimple_seq *pre_p,
                                 gimple_seq *post_p);

static bool lmp_function_ok_for_sibcall (tree, tree);

static bool lmp_frame_pointer_required (void);

static tree lmp_build_builtin_va_list (void);

static bool lmp_legitimate_constant_p (enum machine_mode mode, rtx x);

static bool lmp_legitimate_address_p (enum machine_mode mode, rtx x, bool);

static void lmp_conditional_register_usage (void);

static rtx lmp_legitimize_address (rtx x, rtx oldx,
                                   enum machine_mode mode);

static reg_class_t lmp_secondary_reload (bool, rtx, reg_class_t,
                                         enum machine_mode,
                                         secondary_reload_info *);

static bool lmp_class_likely_spilled_p (reg_class_t);

static void lmp_trampoline_init (rtx m_tramp, tree fndecl, rtx static_chain);

static int lmp_issue_rate (void);

static int lmp_adjust_priority (rtx, int);

static int lmp_adjust_cost (rtx, rtx, rtx, int);

static int lmp_register_move_cost (enum machine_mode, reg_class_t,
                                   reg_class_t);

static int lmp_memory_move_cost (enum machine_mode, reg_class_t, bool);

static bool lmp_rtx_costs (rtx x, int code, int outer_code,
                           int opno, int *total, bool speed);

static void lmp_option_override (void);

static void lmp_init_libfuncs (void);

static unsigned int lmp_reorg (void);
...
#define TARGET_PASS_BY_REFERENCE lmp_pass_by_reference

#undef TARGET_FUNCTION_ARG
#define TARGET_FUNCTION_ARG lmp_function_arg

#undef TARGET_FUNCTION_ARG_ADVANCE
#define TARGET_FUNCTION_ARG_ADVANCE lmp_function_arg_advance

#undef TARGET_RETURN_IN_MEMORY
#define TARGET_RETURN_IN_MEMORY lmp_return_in_memory

#undef TARGET_FUNCTION_VALUE
#define TARGET_FUNCTION_VALUE lmp_function_value

#undef TARGET_LIBCALL_VALUE
#define TARGET_LIBCALL_VALUE lmp_libcall_value

#undef TARGET_SETUP_INCOMING_VARARGS
#define TARGET_SETUP_INCOMING_VARARGS lmp_setup_incoming_varargs

#undef TARGET_EXPAND_BUILTIN_VA_START
#define TARGET_EXPAND_BUILTIN_VA_START lmp_va_start

#undef  TARGET_BUILD_BUILTIN_VA_LIST
#define TARGET_BUILD_BUILTIN_VA_LIST lmp_build_builtin_va_list

#undef  TARGET_GIMPLIFY_VA_ARG_EXPR
#define TARGET_GIMPLIFY_VA_ARG_EXPR lmp_gimplify_va_arg

#undef TARGET_LEGITIMATE_CONSTANT_P
#define TARGET_LEGITIMATE_CONSTANT_P lmp_legitimate_constant_p

#undef TARGET_LEGITIMATE_ADDRESS_P
#define TARGET_LEGITIMATE_ADDRESS_P lmp_legitimate_address_p

#undef  TARGET_ATTRIBUTE_TABLE
#define TARGET_ATTRIBUTE_TABLE lmp_attribute_table

#undef  TARGET_INIT_LIBFUNCS
#define TARGET_INIT_LIBFUNCS lmp_init_libfuncs
...
#define TARGET_SCHED_ISSUE_RATE lmp_issue_rate

#undef  TARGET_SCHED_ADJUST_PRIORITY
#define TARGET_SCHED_ADJUST_PRIORITY lmp_adjust_priority

#undef  TARGET_SCHED_ADJUST_COST
#define TARGET_SCHED_ADJUST_COST lmp_adjust_cost

#undef TARGET_MEMORY_MOVE_COST
#define TARGET_MEMORY_MOVE_COST lmp_memory_move_cost

#undef TARGET_REGISTER_MOVE_COST
#define TARGET_REGISTER_MOVE_COST lmp_register_move_cost

#undef TARGET_RTX_COSTS
#define TARGET_RTX_COSTS  lmp_rtx_costs

#undef  TARGET_FUNCTION_OK_FOR_SIBCALL
#define TARGET_FUNCTION_OK_FOR_SIBCALL lmp_function_ok_for_sibcall

#undef TARGET_FRAME_POINTER_REQUIRED
#define TARGET_FRAME_POINTER_REQUIRED lmp_frame_pointer_required

#undef TARGET_SECONDARY_RELOAD
#define TARGET_SECONDARY_RELOAD lmp_secondary_reload

#undef TARGET_CLASS_LIKELY_SPILLED_P
#define TARGET_CLASS_LIKELY_SPILLED_P lmp_class_likely_spilled_p

#undef TARGET_LEGITIMIZE_ADDRESS
#define  TARGET_LEGITIMIZE_ADDRESS lmp_legitimize_address

#undef TARGET_OPTION_OVERRIDE
#define TARGET_OPTION_OVERRIDE lmp_option_override

#undef TARGET_CONDITIONAL_REGISTER_USAGE
#define TARGET_CONDITIONAL_REGISTER_USAGE lmp_conditional_register_usage

#undef TARGET_TRAMPOLINE_INIT
#define TARGET_TRAMPOLINE_INIT lmp_trampoline_init

struct gcc_target targetm = TARGET_INITIALIZER;

struct rtl_opt_pass pass_lmp_reorg =
{
 {
  RTL_PASS,
  "mach2",				/* name */
...
struct register_pass_info insert_pass_lmp_reorg =
{
  &pass_lmp_reorg.pass,			/* pass */
  "dbr",				/* reference_pass_name */
  1,					/* ref_pass_instance_number */
  PASS_POS_INSERT_AFTER			/* po_op */
};

/* Options override for LMP.  */

static void
lmp_option_override (void)
{
  ...
  if (lmp_cpu_and_features == PROCESSOR_GR6)
    {
  ...
  if (!global_options_set.x_lmp_cpu)
    lmp_cpu = lmp_cpu_and_features;
  ...
  register_pass (&insert_pass_lmp_reorg);
}
...
static int
lmp_adjust_priority (rtx insn, int priority)
{
  ...
  if (lmp_cpu == PROCESSOR_GR5
      && reload_completed
      && INSN_P (insn)
      && recog_memoized (insn) >= 0
      && get_attr_type (insn) == TYPE_REG_MEM)
    return priority + 1;
...
static int
lmp_adjust_cost (rtx insn, rtx link, rtx dep_insn, int cost)
{
  enum attr_type attr_type;

  /* Don't adjust costs for true dependencies as they are described
     with bypasses. */
  ...
  if (REG_NOTE_KIND (link) == REG_DEP_ANTI)
    {
	 ...
	 account for every dependency involving a write.  */
      if (attr_type == TYPE_REG_FP && lmp_cpu == PROCESSOR_GR5)
	{
	  /* INSN is FLOAD. */
	 ...
	 but an anti dependency between them, so we simply reuse it.  */
      else if (attr_type == TYPE_RET && lmp_cpu == PROCESSOR_GR6)
	{
	  rtx dep_pat = PATTERN (dep_insn);
...
static tree
lmp_handle_interrupt_attr (tree *node, tree name,
			   tree args ATTRIBUTE_UNUSED,
			   int flags ATTRIBUTE_UNUSED,
			   bool *no_add_attrs)
{
  const char *txt_name = IDENTIFIER_POINTER (name);

  if (TREE_CODE (*node) != FUNCTION_DECL)
    {
      warning ("`%s' attribute only applies to functions", txt_name);
      *no_add_attrs = true;
    }
...
static bool
lmp_legitimate_constant_p (enum machine_mode mode ATTRIBUTE_UNUSED,
...
unsigned int
lmp_data_alignment (tree type, unsigned int align)
{
...
int
lmp_hard_regno_rename_ok (unsigned int from ATTRIBUTE_UNUSED,
			  unsigned int to)
{
...
static bool
lmp_function_ok_for_sibcall (tree decl ATTRIBUTE_UNUSED,
			     tree exp ATTRIBUTE_UNUSED)
{
  return !lmp_interrupt_function_p ();
}
...
/* This is called with OUT_INSN an instruction setting a (base) register
   ...
   On the original LMP, a pipeline data hazard occurs when the Dest of one
   instruction becomes the SrcA for an immediately following READ or WRITE
   instruction with a non-zero index (indexing occurs at the decode stage
   and so a NOP must be inserted between the instructions for this to work).
...
int
gr5_hazard_bypass_p (rtx out_insn, rtx in_insn)
{
...
/* Insert a NOP wherever there is a pipeline hazard.  */

static void
gr5_avoid_hazard (rtx insn, unsigned int *last_reg_set, int *last_insn_call)
{
  ...
  if (set != NULL_RTX)
	...
	destreg = 0;
	 ...
	 the same register in each case.  */
      if (*last_reg_set != 0 && memrtx != NULL_RTX)
	{
	  unsigned int reg = 0;

	  /* Check for an index (original LMP target).  */
	  if (GET_CODE (memrtx) == PLUS)
	    reg = REGNO (XEXP (memrtx, 0));

	  /* Check for an MCM target or rs := [rs], in DI or DF mode.  */
	  else if (TARGET_MCM || (double_p && REGNO (memrtx) == destreg))
	    reg = REGNO (memrtx);
	  ...
	  if (reg == *last_reg_set)
	    {
	      if (dump_file)
	        fprintf (dump_file, "inserting nop before insn %d\n",
			 INSN_UID (insn));
...
/* Go through the instruction stream and insert nops where necessary to avoid
   pipeline hazards.  There are two cases:
     ...
     1. On the original LMP, it is invalid to set the value of a (base)
	register and then use it in an address with a non-zero index in
	the next instruction.  See the LAe2000 User's Guide, page 12.
...
{
  rtx insn;
  unsigned int last_reg_set = 0;
  int last_insn_call = 0;
  ...
  for (insn = get_insns (); insn != 0; insn = NEXT_INSN (insn))
      ...
      if (INSN_P (insn))
	{
	  if (GET_CODE (PATTERN (insn)) == SEQUENCE)
		...
		gr5_avoid_hazard (XVECEXP (PATTERN (insn), 0, i),
				  &last_reg_set, &last_insn_call);
	    }

	  else if (GET_CODE (insn) == CALL_INSN)
	    /* This call is going to get a nop in its delay slot.  */
	    last_insn_call = last_reg_set = 0;

	  else
	    gr5_avoid_hazard (insn, &last_reg_set, &last_insn_call);
	}
...
/* Return true if INSN is an active non-jump instruction.  */

static bool
active_nonjump_insn_p (const_rtx insn)
{
  enum rtx_code code;
  ...
  code = GET_CODE (PATTERN (insn));
  if (code == SEQUENCE || code == CLOBBER || code == USE)
    return false;

  return true;
}

/* Scan the RTL stream and try to hoist the compare instructions profitably.

   Since we use cc0, the compare instructions associated with branches are
   never separated from the branch.  By hoisting them ahead of an unrelated
   EAM to register move, they can be executed during the latency period of
   the preceding EAM operation, typically a mult, thus potentially saving
   1 cycle if there aren't enough instructions to fill the latency window.
   Similarly, on the GR5, by hoisting them between an instruction that sets
   a (base) register and the next instruction using it in an address, they
   can be executed in lieu of a nop generated to avoid the pipeline hazard,
   thus saving again 1 cycle.  */

static void
lmp_hoist_compare_insns (void)
{
  rtx insn;
  ...
  for (insn = get_last_insn (); insn != NULL_RTX; insn = PREV_INSN (insn))
	...
	continue;
      ...
      /* We have a compare insn.  Scan backward to find out up to which
	 point it could be profitably hoisted.  */
      earliest = insn;

      while (true)
        {
	  /* Skip notes and debug insns but stop at non-jump insns.  */
	  rtx prev = prev_nonnote_nondebug_insn (earliest);
	  if (!prev || !active_nonjump_insn_p (prev))
	    break;

	  /* For a non-jump insn, cc0 may not be changed and, if a register
	     is changed, it may not be mentioned in the compare insn.  */
	  cc = get_attr_cc (prev);
	  if (cc == CC_UNCHANGED)
	    ;
	  else if (cc == CC_CHANGE0)
	    {
	      enum attr_type type = get_attr_type (prev);
	      rtx dest;

	      /* Stop at EAM operations with long latency since we are doing
		 this precisely to fill the latency window.  */
	      if (type == TYPE_DIV || type == TYPE_DIVD || type == TYPE_MUL)
		break;

	      /* Stop if PREV->EARLIEST constitutes a pipeline hazard.  */
	      type = get_attr_type (earliest);
	      if (lmp_cpu == PROCESSOR_GR5
		  && (type == TYPE_MEM_REG || type == TYPE_REG_MEM)
		  && gr5_hazard_bypass_p (prev, earliest))
		break;

	      dest = SET_DEST (single_set (prev));
	      if (GET_CODE (dest) == STRICT_LOW_PART)
		dest = XEXP (dest, 0);
      ...
      /* Hoist the compare insn if we have found a suitable place.  We need to
	 do it manually because we have SEQUENCEs in the RTL stream.  */
      if (earliest != insn)
	{
	  if (dump_file)
	    fprintf (dump_file, "hoisting compare insn %d\n", INSN_UID (insn));
	  NEXT_INSN (PREV_INSN (insn)) = NEXT_INSN (insn);
	  PREV_INSN (NEXT_INSN (insn)) = PREV_INSN (insn);
	  if (NONJUMP_INSN_P (NEXT_INSN (insn))
	      && GET_CODE (PATTERN (NEXT_INSN (insn))) == SEQUENCE)
	    PREV_INSN (XVECEXP (PATTERN (NEXT_INSN (insn)), 0, 0))
	      = PREV_INSN (insn);
	  NEXT_INSN (insn) = earliest;
	  PREV_INSN (insn) = PREV_INSN (earliest);
	  NEXT_INSN (PREV_INSN (earliest)) = insn;
	  PREV_INSN (earliest) = insn;
	}
    }
}
...
lmp_reorg (void)
{
  /* The compare instructions hoisting pass is part of scheduling.  */
  if (optimize > 0 && flag_schedule_insns_after_reload)
    lmp_hoist_compare_insns ();
...
static bool
lmp_pass_by_reference (cumulative_args_t ca ATTRIBUTE_UNUSED,
...
static rtx
lmp_function_arg (cumulative_args_t pcum_v, enum machine_mode mode,
...
/* Update the summarizer variable pointed to by PCUM_V to advance past an
   ...
   analyzing the _following_ argument with lmp_function_arg.  */

static void
lmp_function_arg_advance (cumulative_args_t pcum_v,
			  enum machine_mode mode,
...
static bool
lmp_return_in_memory (const_tree type, const_tree fntype ATTRIBUTE_UNUSED)
{
...
static rtx
lmp_function_value_1 (enum machine_mode mode)
{
...
static rtx
lmp_function_value (const_tree ret_type,
		    const_tree fn_decl_or_type ATTRIBUTE_UNUSED,
		    bool outgoing ATTRIBUTE_UNUSED)
{
  return lmp_function_value_1 (TYPE_MODE (ret_type));
}
...
static rtx
lmp_libcall_value (enum machine_mode mode, const_rtx fun ATTRIBUTE_UNUSED)
{
  return lmp_function_value_1 (mode);
}
...
static void
lmp_setup_incoming_varargs (cumulative_args_t pcum_v, enum machine_mode mode,
			    ...
			    int no_rtl)
{
  ...
  if (!no_rtl && size > 0)
    {
	 ...
	 the LMP, we create a base register for the pretend args.  */
      rtx ptr
	= force_reg (Pmode, plus_constant (virtual_incoming_args_rtx, -size));
  ...
  lmp_reg_parm_save_area_size = size;
}

/* Define the `__builtin_va_list' type for the ABI.  */

static tree
lmp_build_builtin_va_list (void)
{
...
static void
lmp_va_start (tree valist, rtx nextarg ATTRIBUTE_UNUSED)
{
...
static tree
lmp_gimplify_va_arg (tree valist, tree type, gimple_seq *pre_p,
		     gimple_seq *post_p)
{
  tree f_ovfl, f_gbase, f_fbase, f_gbytes, f_fbytes;
  ...
  if (by_reference_p)
    {
      t = lmp_gimplify_va_arg (valist, ptrtype, pre_p, post_p);
...
static bool
lmp_legitimate_address_p (enum machine_mode mode, rtx x, bool strict)
{
  rtx base;
  unsigned int regno;
...
static rtx
lmp_legitimize_address (rtx x, rtx oldx ATTRIBUTE_UNUSED,
			enum machine_mode mode)
{
  if (GET_CODE (x) == PLUS
      ...
      if (offset_base != 0 && offset_base != offset && (offset & mask1) == 0)
	{
	  ...
	  return plus_constant (ptr_reg, offset - offset_base);
	}
    }
...
/* Perform a similar function to lmp_legitimize_address, but this time
   for reload. Generating new registers is not an option here. Parts
   that need reloading are indicated by calling push_reload. */

rtx
lmp_legitimize_reload_address (rtx x, enum machine_mode mode, int opnum,
...
static int
lmp_register_move_cost (enum machine_mode mode, reg_class_t from,
			reg_class_t to)
{
...
/* Return the cost of moving data of mode MODE between a register of class
   ...
   lmp_register_move_cost.  */

static int
lmp_memory_move_cost (enum machine_mode mode, reg_class_t to ATTRIBUTE_UNUSED,
		      bool in)
{
...
static bool
lmp_rtx_costs (rtx x, int code, int outer_code ATTRIBUTE_UNUSED,
...
void
lmp_expand_copysign (rtx *operands, enum machine_mode mode)
{
  rtx dest = operands[0];
  rtx op0 = operands[1];
  rtx op1 = operands[2];
...
void
lmp_expand_int_cstore (rtx *operands, enum machine_mode mode)
{
  ...
  /* Emit the initial comparison.  */
  emit_insn (gen_rtx_SET (VOIDmode, cc0_rtx,
			  gen_rtx_COMPARE (VOIDmode, op1, op2)));

  /* Then we need either a single ADC or a SUBC and a PLUS.  */
  sltu = gen_rtx_LTU (SImode, cc0_rtx, const0_rtx);

  if (reverse)
    {
      rtx tmp = copy_to_mode_reg (SImode, gen_rtx_NEG (SImode, sltu));
      emit_insn (gen_rtx_SET (VOIDmode, op0,
			      gen_rtx_PLUS (SImode, tmp, const1_rtx)));
    }
  else
...
void
lmp_expand_fp_cstore (rtx *operands, enum machine_mode mode ATTRIBUTE_UNUSED)
{
  enum rtx_code code = GET_CODE (operands[1]);
  ...
  /* Then we need either a single ADC or a SUBC and a PLUS.  */
  slt = gen_rtx_LT (SImode, cc0_rtx, const0_rtx);
...
int
lmp_expand_block_move (rtx *operands)
{
  rtx dst = operands[0];
  rtx src = operands[1];
...
int
lmp_expand_block_set (rtx *operands)
{
  rtx dst = operands[0];
...
static void
lmp_trampoline_init (rtx m_tramp, tree fndecl, rtx static_chain)
{
  ...
  emit_move_insn (gen_rtx_MEM (SImode, plus_constant (addr, 0)),
		  plus_constant (expand_shift (RSHIFT_EXPR, SImode, fnaddr,
					       16, NULL_RTX, 1),
				 0x04a90000));

  emit_move_insn (gen_rtx_MEM (SImode, plus_constant (addr, 4)),
		  plus_constant (expand_and (SImode, fnaddr, GEN_INT (0xffff),
					     NULL_RTX),
				 0x04890000));

  emit_move_insn (gen_rtx_MEM (SImode, plus_constant (addr, 8)),
		  plus_constant (expand_shift (RSHIFT_EXPR, SImode,
					       static_chain,
					       16, NULL_RTX, 1),
				 0x04b40000));

  emit_move_insn (gen_rtx_MEM (SImode, plus_constant (addr, 12)),
		  gen_int_mode (0xff892404, SImode));

  emit_move_insn (gen_rtx_MEM (SImode, plus_constant (addr, 16)),
		  plus_constant (expand_and (SImode, static_chain,
...
static bool
lmp_frame_pointer_required (void)
{
...
void
lmp_profile_hook (void)
{
  lmp_frame_needed = true;
...
rtx
lmp_dynamic_chain_address (rtx frame)
{
...
rtx
lmp_return_addr_rtx (int count, rtx frame ATTRIBUTE_UNUSED)
{
  ...
  lmp_frame_needed = true;
  return gen_frame_mem (Pmode, plus_constant (hard_frame_pointer_rtx, 4));
}
...
rtx
lmp_eh_return_handler_rtx (void)
{
  rtx mem = gen_frame_mem (SImode, plus_constant (hard_frame_pointer_rtx, 4));
  MEM_VOLATILE_P (mem) = 1;
  return mem;
}

static struct machine_function *
lmp_init_machine_status (void)
{
  return (struct machine_function *) ggc_alloc_cleared_machine_function ();
}
...
void
lmp_init_expanders (void)
{
  init_machine_status = lmp_init_machine_status;
}

/* cc0 condition code handling. */

#define CC_FP_COMPARE  0x08000
#define CC_BIT_COMPARE 0x10000
...
notice_update_cc (rtx exp, rtx insn)
{
  switch (get_attr_cc (insn))
    ...
    case CC_SET_NOOV:
	 ...
	 for which the altered operator doesn't exist on the LMP.  */
      cc_status.value1 = SET_DEST (exp);
    ...
    case CC_COMPARE:
      CC_STATUS_INIT;
      /* We don't record the COMPARE itself like in final_scan_insn to
	 speed things up.  Redundant COMPARE insns shouldn't exist.  */
      if (XEXP (SET_SRC (exp), 1) == const0_rtx)
	{
	  cc_status.value1 = XEXP (SET_SRC (exp), 0);

	  /* If this is a btst, the result is in C instead of Z.  */
	  if (GET_CODE (XEXP (SET_SRC (exp), 0)) == ZERO_EXTRACT)
	    cc_status.flags |= CC_BIT_COMPARE;
	}
      break;

    case CC_COMPARE_FP:
      CC_STATUS_INIT;
      /* Likewise and the FP arithmetic insns don't set flags.  */
      cc_status.flags |= CC_FP_COMPARE;
      break;

    case CC_CHANGE0:
      /* We need to check that the recorded value isn't changed.  */
...
/* Return the code of the comparison operator in the next CC0 user.  */

static enum rtx_code
next_cc_user_code (rtx insn)
{
  rtx cc_user = next_cc0_user (insn);

  gcc_assert (cc_user != NULL_RTX);

  if (JUMP_P (cc_user)
      && GET_CODE (PATTERN (cc_user)) == SET
      && GET_CODE (SET_SRC (PATTERN (cc_user))) == IF_THEN_ELSE)
    return GET_CODE (XEXP (SET_SRC (PATTERN (cc_user)), 0));

  if (INSN_P (cc_user) && GET_CODE (PATTERN (cc_user)) == SET)
    {
      rtx src = SET_SRC (PATTERN (cc_user));

      if (comparison_operator (src, VOIDmode))
	return GET_CODE (src);

      /* This is neg_slt.  */
      if (GET_CODE (src) == NEG && GET_CODE (XEXP (src, 0)) == LT)
	return UNGE;

      /* This is plus_slt.  */
      if (GET_CODE (src) == PLUS && GET_CODE (XEXP (src, 1)) == LT)
	return LT;

      /* This is minus_slt.  */
      if (GET_CODE (src) == MINUS && GET_CODE (XEXP (src, 1)) == LT)
	return UNGE;
    }
...
/* Return whether the next CC0 user can raise an IEEE exception.  */

static int
next_cc_user_exception (rtx insn)
{
  switch (next_cc_user_code (insn))
    ...
    /* These 4 comparison codes are for integers only.  */
    case GEU:
    case GTU:
    case LEU:
    case LTU:
...
/* Output a FP compare of OP0 and OP1.  INSN is the instruction.  */

const char *
output_fp_compare (rtx op0, rtx op1, rtx insn)
{
  enum machine_mode mode = GET_MODE (op0);
  rtx operands[2];

  if (op0 == CONST0_RTX (mode))
    operands[0] = regno_reg_rtx [FP_REG_FIRST]; /* f0 */
  else
    operands[0] = op0;
  if (op1 == CONST0_RTX (mode))
    operands[1] = regno_reg_rtx [FP_REG_FIRST]; /* f0 */
  else
    operands[1] = op1;

  if (next_cc_user_exception (insn))
    output_asm_insn ("fcmpe   r0,%0,%1", operands);
  else
    output_asm_insn ("fcmp    r0,%0,%1", operands);

  return "";
}

/* Branch instructions on the LMP.
...
static const char *
output_branch (rtx label, const char *cond, rtx insn)
{
  char str[64];
  rtx operands[2];
...
const char *
output_ubranch (rtx label, rtx insn)
{
  return output_branch (label, "tr", insn);
}

/* Output a conditional branch to LABEL.  OP is the conditional expression.
   REVERSED is non-zero if we should reverse the sense of the comparison.
   INSN is the instruction.  */

const char *
output_cbranch (rtx label, rtx op, int reversed, rtx insn)
{
  ...
  if (reversed)
    {
      if (cc_prev_status.flags & CC_FP_COMPARE)
	code = reverse_condition_maybe_unordered (code);
      else
	code = reverse_condition (code);
    }
  ...
  switch (code)
    {
    case NE:
      if (cc_prev_status.flags & CC_BIT_COMPARE)
	cond = "cs";
      else
	cond = "ne";
      break;
...
print_operand (FILE *file, rtx op, int letter)
{
  ...
  switch (letter)
    ...
    case 'b':
      ...
      fprintf (file, "%lu", UINTVAL (op) & 0xff);
      return;

    case 'w':
      /* Print LS 16 bits of operand.  */
      fprintf (file, "%lu", UINTVAL (op) & 0xffff);
      return;

    case 'u':
      /* Print MS 16 bits of operand.  */
      fprintf (file, "%lu", (UINTVAL (op) >> 16) & 0xffff);
      return;
  ...
  switch (GET_CODE (op))
    ...
    case MEM:
      lmp_output_address (file, GET_MODE (op), XEXP (op, 0));
      break;
...
static void
lmp_output_address (FILE *file, enum machine_mode mode, rtx addr)
{
	...
	switch (GET_CODE (x))
	  ...
	  case SUBREG:
	    ...
	    switch (GET_CODE (y))
	      ...
	      case CONST_INT:
		/* e.g.  read r1,10(r2) 
		   The address is "10(r2)".  */
		switch (mode)
		  {
		  case QImode:
		    fprintf (file, "%ld(%s)", INTVAL (y), reg_names[regno]);
		    break;

		  case HImode:
		    fprintf (file, "%ld(%s)", INTVAL (y) >> 1,
			     reg_names[regno]);
		    break;

		  case SImode:
		    fprintf (file, "%ld(%s)", INTVAL (y) >> 2,
			     reg_names[regno]);
		    break;
...
{
  lmp_output_address (file, QImode, addr);
}

/* The LMP stack frames look like:
...
const int first_regno[last_type] = {0, MDB_REGNUM, MDC_REGNUM, FP_REG_FIRST};

/* Size in bytes of each register type.  */
const int reg_type_size[last_type] = {4, 8, 4, 4};

/* Structure to be filled in by lmp_compute_frame_size.  */
struct lmp_frame_info
{
...
/* Current frame information calculated by lmp_compute_frame_size.  */
static struct lmp_frame_info current_frame_info;
...
static int
lmp_save_reg_p (int interrupt, int regno)
{
  switch (regno)
    {
...
/* Compute the frame size required by the function.  This function is called
   during the reload pass and also by lmp_expand_prologue.  */

static int
lmp_compute_frame_size (int size)
{
  const int save_area_size = lmp_reg_parm_save_area_size;
  const int var_size = LMP_STACK_ALIGN (size);
  const int save_fp
    = frame_pointer_needed || df_regs_ever_live_p (HARD_FRAME_POINTER_REGNUM);
  const int save_lr = frame_pointer_needed || !current_function_is_leaf;
  ...
  const int interrupt = lmp_interrupt_function_p ();
  ...
  for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
    {
      if (lmp_save_reg_p (interrupt, regno))
	{
...
int
lmp_initial_elimination_offset (int from, int to ATTRIBUTE_UNUSED)
{
  const int frame_size = lmp_compute_frame_size (get_frame_size ());
...
static void
lmp_save_regs (int alloc, int offset, int low_regno, int high_regno)
{
  ...
  emit_frame_insn (gen_addsi3 (stack_pointer_rtx, stack_pointer_rtx,
			       GEN_INT (-alloc)));
	  ...
	  switch (reg_type)
		...
		rtx mem
		  = gen_frame_mem (DImode,
				   plus_constant (stack_pointer_rtx, offset));
		MEM_VOLATILE_P (mem) = volatile_p;
		emit_insn (gen_movdi (tmp, gen_rtx_REG (DImode, regno)));
		/* Do not generate CFI if in interrupt handler.  */
		if (volatile_p)
		  insn = emit_insn (gen_movdi (mem, tmp));
		else
		  {
		    insn = emit_frame_insn (gen_movdi (mem, tmp));
		    add_reg_note (insn, REG_FRAME_RELATED_EXPR,
				  gen_rtx_SET (DImode, mem,
		...
		rtx mem
		  = gen_frame_mem (SImode,
				   plus_constant (stack_pointer_rtx, offset));
		MEM_VOLATILE_P (mem) = volatile_p;
		emit_insn (gen_movsi (tmp, gen_rtx_REG (SImode, regno)));
		insn = emit_frame_insn (gen_movsi (mem, tmp));
		add_reg_note (insn, REG_FRAME_RELATED_EXPR,
			      gen_rtx_SET (SImode, mem,
		...
		rtx mem
		  = gen_frame_mem (SFmode,
				   plus_constant (stack_pointer_rtx, offset));
		MEM_VOLATILE_P (mem) = volatile_p;
		emit_insn (gen_movsf (tmp, gen_rtx_REG (SFmode, regno)));
		insn = emit_frame_insn (gen_movsf (mem, tmp));
		add_reg_note (insn, REG_FRAME_RELATED_EXPR,
			      gen_rtx_SET (SFmode, mem,
...
lmp_expand_prologue (void)
{
  ...
  if (reg_size2)
    {
      lmp_save_regs (reg_size1 + save_area_size, reg_size1, 0, max_reg1);
      reg_size = reg_size2;
  ...
  if (reg_size && !combine)
    lmp_save_regs (fsize - local_frame_offset - var_size, reg_size,
  ...
  /* Otherwise space may still need to be allocated for the variables.  */
      ...
      if (alloc_size > 65535)
	{
	  ...
	  stack_alloc = emit_frame_insn (gen_subsi3 (stack_pointer_rtx,
						     stack_pointer_rtx, tmp));
	  add_reg_note (stack_alloc, REG_FRAME_RELATED_EXPR,
  ...
  if (save_lr)
	...
	base_rtx = stack_pointer_rtx;
      ...
      mem = gen_frame_mem (SImode,
			   plus_constant (base_rtx, save_fp * UNITS_PER_WORD));
...
static void
lmp_restore_regs (int dealloc, int offset, int high_regno, int low_regno)
{
  /* If this is an interrupt handler function, then mark the register
  ...
  for (regno = high_regno; regno >= low_regno; --regno)
      ...
      if (current_frame_info.mask[reg_type] & mask_bit)
	{
	  switch (reg_type)
		...
		emit_move_insn (gen_rtx_REG (DImode, regno), tmp);
	      }
	      break;
		...
		MEM_VOLATILE_P (mem) = volatile_p;
		emit_move_insn (tmp, mem);
		emit_move_insn (gen_rtx_REG (SImode, regno), tmp);
	      }
	      break;
		...
		MEM_VOLATILE_P (mem) = volatile_p;
		emit_move_insn (tmp, mem);
		emit_move_insn (gen_rtx_REG (SFmode, regno), tmp);
	      }
	      break;
  ...
  emit_insn (gen_stack_pop (GEN_INT (dealloc)));
}

/* This function generates the code for function exit.  */

void
lmp_expand_epilogue (void)
{
  const int save_area_size = current_frame_info.save_area_size;
  const int reg_size1 = current_frame_info.reg_size1;
  const int max_reg1 = current_frame_info.max_reg1;
  ...
  if (restore_fp)
    {
      if (TARGET_MCM && !current_function_sp_is_unchanging)
	emit_move_insn (hard_frame_pointer_rtx,
			gen_frame_mem (SImode, hard_frame_pointer_rtx));
      else
	emit_move_insn (hard_frame_pointer_rtx,
  ...
  if (restore_lr)
    emit_move_insn (gen_rtx_REG (SImode, LINK_REGNUM),
		    gen_frame_mem (SImode,
				   plus_constant (stack_pointer_rtx,
						  restore_fp
						    * UNITS_PER_WORD)));
  ...
  if (reg_size && combine)
    lmp_restore_regs (fsize, local_frame_offset + var_size,
  ...
  /* Otherwise deallocate the variables first.  */
      ...
      if (pop_size > 65535)
	{
	  ...
	  emit_insn (gen_stack_pop (tmp));
        }
      else
	emit_insn (gen_stack_pop (GEN_INT (pop_size)));
    }
  ...
  if (reg_size && !combine)
    lmp_restore_regs (fsize - local_frame_offset - var_size, 0,
		      FIRST_PSEUDO_REGISTER - 1, last_reg);

  /* If the first block of registers has yet to be restored, do it now.  */
  if (reg_size2)
    lmp_restore_regs (reg_size1 + save_area_size, 0, max_reg1, 0);
...
bool
lmp_can_use_return_insn_p (void)
{
...
static reg_class_t
lmp_secondary_reload (bool in_p ATTRIBUTE_UNUSED, rtx x,
		      reg_class_t rclass,
		      enum machine_mode mode ATTRIBUTE_UNUSED,
...
static bool
lmp_class_likely_spilled_p (reg_class_t rclass ATTRIBUTE_UNUSED)
{

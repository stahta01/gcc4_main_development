	...
	case VECTOR_CST:
	  {
	    ...
	    tree link = TREE_VECTOR_CST_ELTS (exp);
	    output_constant_1 (TREE_VALUE (link), elt_size, align, reverse);
	    thissize = elt_size;
	    while ((link = TREE_CHAIN (link)) != NULL_TREE)
	      {
		output_constant_1 (TREE_VALUE (link), elt_size, nalign, reverse);
		thissize += elt_size;
	      }
	    break;
	  }
